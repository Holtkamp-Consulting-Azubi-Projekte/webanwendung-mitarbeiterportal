...
const API_URL = "http://backend:5050/api/time-entries/";
const PROFILE_API_URL = "http://backend:5050/api/profile"; // API-URL für Profil
...
const Zeitmatrix = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const response = await axios.get('http://backend:5050/api/profile', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
